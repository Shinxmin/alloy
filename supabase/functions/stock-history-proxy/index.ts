...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof body?.ticker === "string") ticker = body.ticker;
  ...
  if (!ticker || currency !== "KRW") {
    return new Response(JSON.stringify({ supported: false, history: [] }), {
  ...
  if (!krxApiKey) {
    ...
    return new Response(JSON.stringify({ supported: true, history: [] }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  const history = results
    ...
    .map((r) => ({ time: r.iso, value: r.price }));
